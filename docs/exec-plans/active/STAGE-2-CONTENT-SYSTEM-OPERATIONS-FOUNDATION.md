...
Implemented 2026-09-26. `pnpm content:release -- --pack <id> --version <n> --actor
<fixture-id>` and `pnpm content:verify [-- --trusted-commit <full-sha>]` are implemented as
dependency-injected runners in `content/commands/` (thin `scripts/content-*.ts` wrappers) over the
shared repository store, and both are wired into `verify:full`, so source/artifact drift now fails
the standard verification entry point that CI runs.

`content/schemas/release-bundle.ts` adds the strict, generated-schema-backed release bundle:
`schemaVersion`, the canonical Pack, the Burmese localization, the released version/digests, the
fixture classification, `releasedAt`, and the complete Pack provenance-log prefix ending at that
version's `artifact-released` event. `content/artifacts.ts` is the pure builder
(`buildReleaseArtifacts`, `provenancePrefixThrough`, `assertBundleBindings`): it takes validated
content, gate results, and the sealed prefix, never reads a clock or the filesystem, and serializes
through the existing sorted-key, two-space formatter with a trailing newline, hashing the exact
emitted bytes. Qualification documents, attestation `note` text, and private retirement reasons are
absent; provenance `actorId` values are present but restricted to synthetic `fixture-` identities by
`assertFixtureOnlyProvenance` on the consumption path.

`content/snapshot-index.ts` is now the single index authority (`canonicalArtifactPath`,
`compareArtifactPaths`, `sortSnapshotIndexEntries`, `buildSnapshotIndex`, `snapshotIndexBytes`), used
by release, retirement, repository verification, and the trusted-snapshot verifier, so all four
surfaces agree on canonical byte rendering and locale-independent code-unit ordering. Every
artifact-relative path resolves through `content/store.ts` helpers. `content/governance.ts` is the
shared governance seam (`verifyVersionGovernance`, `assertReleaseManifestBindsVersion`); `content:status`,
`content:release`, and `content:verify` all depend on it, so release cannot commit artifacts into a
state those commands would immediately reject. `content/repository-verify.ts` re-evaluates each
historical release's gates at its recorded release time, reconstructs the sealed prefix, and compares
regenerated bytes, then requires exact bidirectional agreement between release/retirement history,
index entries, and artifact files. A repository with no releases and no artifact files passes and
reports zero releases explicitly. `content/snapshot-verify.ts` exports `requireFullCommitSha`, `verifyArtifactSnapshot`,
and `loadReleasedBundle` as the `content` public surface, plus an `inspectSnapshotVersion` helper that
the test suite deep-imports to exercise the retirement-notice path (refusal lives only in
`loadReleasedBundle`); trust is never inferred from HEAD, a branch name,
an abbreviated id, or local digest agreement, Git is invoked with argv arrays and
`--no-replace-objects` plus a neutralized `GIT_*` environment, the local inventory is compared
file-for-byte against the pinned tree (rejecting symlinks, unexpected files, missing entries, and
byte/digest mismatches), and `loadReleasedBundle` refuses a retired version even though the snapshot
itself still verifies.

The release command loads source, localization, provenance, attestations, and the approving
practitioner's eligibility through the shared seam, accepts standing `practitioner-reviewed` or
`artifact-eligible`, rejects a release timestamp preceding the target version's eligibility event,
reevaluates every gate at one injected/current release timestamp, appends `artifact-eligible` only when
needed and then `artifact-released`, requires fixture-only content and `fixture-` actors, and has no
production-classification override. It refuses existing bundle/manifest files, conflicting or
inconsistent index entries, non-canonical index bytes, and retired versions, then builds every output
before writing: provenance first, exclusive bundle and manifest creation next, index last, through
`commitWrites` with the existing in-process rollback. `pnpm test` grew from 301 to 397 tests,
including byte-identical generation and reconstruction, stable index ordering across multiple Packs
and versions (including multi-digit versions), every gate refusal, duplicate release, injected write
failures on both a first and a second release, source/localization/provenance/bundle/manifest/index
tampering including whitespace-only changes, retirement-notice determinism and negative digest
binding, temporary Git repositories proving invalid pins fail, artifact loading without source files,
retirement blocking loading, and both retirement-deletion variants failing against the trusted
snapshot. The retirement integration tests in `tests/content-cli-lifecycle.test.ts` now use real
release-command output instead of hand-written placeholder artifacts.

Four-discipline review (product-integrity, architecture, security/privacy, test) ran before this
record. Their material findings were addressed: canonical-path and identity binding plus provenance
re-derivation on the artifact-only consumer path; `loadSnapshotVersion` renamed to `inspectSnapshotVersion` and removed from the `content` public
surface, so refusal lives only in `loadReleasedBundle`; `content:release` moved onto
the shared governance seam; a released version with a missing manifest now fails in the seam;
`--no-replace-objects` and a neutralized Git environment; the vacuous self-referential provenance head
pin replaced by an anchor on the release manifest's `releasedAt` with the residual limit stated in
code; a real-directory check plus depth and file-size bounds on the artifact walk; unexpected errors
mapped to exit 1; the runner's verification self-test pinned; and new tests for the pinned-tree byte
comparison, the consumer notice bindings, Git replace refs and environment redirection, the symlinked
artifact root, the depth bound, and the fail-closed error mapping. `pnpm agent:doctor`,
`pnpm verify:fast`, `pnpm verify:invariants`, `pnpm content:schemas:check`, and `pnpm verify:full`
(397 tests) all passed on 2026-09-26. S2-08 remains next; Stage 2 remains ACTIVE and Stage 3
PLANNED.
...
- Historical repository verification re-evaluates the practitioner gate against the **current**
  eligibility record. The time-scoped `validFrom`/`validUntil` window is evaluated at the recorded
  release time, so a later expiry does not break a historical rebuild (tested), but a later
  deactivation, re-verification, or occupation amendment does. S2-04 deliberately made eligibility
  repository-governed, occupation/time-scoped, and free of pack coordinates, and it is not sealed into
...
- Trusted-root verification currently requires a local checkout whose artifact tree matches the
  pinned commit. That is not a distribution, refresh, or trust-acquisition choice, but any later
  distribution decision must revisit `verifyArtifactSnapshot` and inherit this constraint knowingly.
- The release manifest's `localizationApproved: true` is not paired with a per-field marker saying the
  fluent-Burmese evidence behind it is synthetic. This was deliberately not changed in S2-07:
  `classification: "fixture"` and `fixtureOnly: true` are enforced at the schema, command, and
  consumption layers and already carry that meaning mechanically, the synthetic `fixture:` localization
  evidence never crosses the artifact boundary at all, and the S2-06-owned
  `releaseGatesSchema`/`releaseGateManifestFields` projection was left unchanged rather than silently
  widened. If a later stage ever emits non-fixture artifacts, a symmetric marker would have to be added
  before that point.
- The artifact-only consumer can prove the released version's provenance prefix is self-consistent,
  digest-pinned, lifecycle-legal, and anchored to the release manifest, but it cannot prove the prefix
  is *complete* for earlier versions of the same Pack; that requires the authoring sources and is the
  repository verifier's job. Stated in code at the check and in this plan rather than claimed as
  artifact-side detection.
- The release boundary enforces YWAY-P002 through the attested reviewer confirmation
  (`exposureBeforeCommitmentConfirmed` on approved founder/practitioner attestations), not by text
...
- 2026-09-23: S2-03: event digests are sealed over the event's full field set excluding `eventDigest`, so verification recomputes from the stored fields directly and a partially resealed chain still fails on link or lifecycle checks. Provenance verification runs lifecycle transition validation as part of chain verification, so a fully resealed log with an illegal status order is still rejected; hashes detect tampering but do not authenticate actor identity (identity/authorization remain deferred per YWAY-D003). Tail truncation of an otherwise valid chain is undetectable from the log alone; `expectedHeadEventDigest` lets a caller pin the head, and S2-07 anchors the artifact-side check on the release manifest's `releasedAt` plus the trusted Git snapshot root. A committed provenance log is still not independently head-anchored, so an audit-chain-only consumer must pin the head itself.
...
- 2026-09-26: S2-07 added `content/schemas/release-bundle.ts` as the strict, generated-schema-backed
  release boundary (schemaVersion, canonical Pack, Burmese localization, released version/digests,
  fixture classification, `releasedAt`, and the sealed provenance prefix ending at the version's
  `artifact-released` event) and `content/artifacts.ts` as a pure builder. The builder receives
  validated content, gate results, and the sealed prefix and never reads a clock or later provenance,
  so rebuilding a historical release is reproducible. `content/snapshot-index.ts` became the single
  index authority shared by release, retirement, repository verification, and the trusted-snapshot
  verifier; ordering is code-unit (`compareArtifactPaths`), not `localeCompare`, because a host
  collation would make the committed index differ between machines. It is lexical, not numeric, so
  `10` sorts before `2`; that is deterministic and test-pinned but recorded because a numeric
  expectation would be a silent trap.
- 2026-09-26: S2-07 extracted `content/governance.ts` from `content/commands/status.ts` so
  `content:status`, `content:release`, and `content:verify` share one version-scope, attestation/event
  bijection, retirement-record, notice, and release-manifest-binding implementation. `content:release`
  now runs the same seam, so a release cannot commit artifacts into a state `content:status` or
  `content:verify` would immediately reject. A released version with a missing release manifest is now
  refused by the seam rather than tolerated.
- 2026-09-26: S2-07 added `content/repository-verify.ts` (rebuild every historical release at its
  recorded release time, compare regenerated bytes, require exact bidirectional agreement between
  history, index entries, and artifact files) and `content/snapshot-verify.ts`
  (`requireFullCommitSha`, `verifyArtifactSnapshot`, `loadReleasedBundle`). Git is invoked with argv
  arrays, `shell: false`, `--no-replace-objects`, a neutralized `GIT_*` environment, a bounded buffer
  and timeout, and NUL-delimited `ls-tree` with mode/type rejection. The `--no-replace-objects` flag
  and the environment neutralization are load-bearing: without them a local `refs/replace/<commit>`
  entry or an inherited `GIT_DIR`/`GIT_OBJECT_DIRECTORY` would let local `.git` state substitute for
  the caller-supplied trusted root, which is the exact property YWAY-D003 relies on for meaningful
  retirement-notice absence. A test registers a replace ref and proves the verifier still reads the
  real pinned tree.
- 2026-09-26: S2-07 wired `content:schemas:check` and `content:verify` into `verify:full` (which CI
  runs) and made the runner fail closed when the `test` script is missing, so generated-schema drift,
  content/artifact drift, and a silently skipped suite can no longer pass verification. At S2-07 the
  repository still has no committed `artifacts/` or `content/packs/`, so `content:verify` currently
  verifies an empty repository; S2-08 supplies the live coverage.
- 2026-09-26: S2-07 security review found the artifact-only provenance head pin was self-referential
  (`expectedHeadEventDigest` read from the log being verified, so it could never fail) and that
  `listRegularFiles` never checked the artifact root was a real directory, had no walk-depth or
  file-size bound, and that an unexpected error type escaped as a raw crash. The pin is replaced by an
  anchor on the separately digest-pinned release manifest's `releasedAt`, the residual limit is stated
  in code (an artifact-only consumer cannot prove a re-sealed prefix is complete for *earlier*
  versions of the same Pack — that needs the authoring sources), the root is `lstat`ed, depth and
  size bounds were added, and `failureResult` now maps any `Error` to exit 1 while exit 2 stays
  reserved for usage errors.
- 2026-09-26: S2-07 test review found that five tests were duplicated verbatim in one file, which
  would have inflated the recorded test count as completion evidence. The duplicates were removed and
  the suite re-run; the recorded count is the corrected one. Review also flagged that the
  commitment-first `nextFork` in the YWAY-P002 refusal test was decorative, because enforcement is an
...
| 2026-09-26 | Implement S2-07 as a strict generated release-bundle schema plus a pure artifact builder, a shared locale-independent snapshot-index authority, a shared version-governance seam for status/release/verify, a repository verifier that rebuilds historical releases, and a Git-trusted-snapshot verifier with a consumer loader | Bounded implementation detail under ACCEPTED YWAY-D003; no new product rule and no deferred architecture choice |
| 2026-09-26 | Require `--actor` on `content:release` and add `content:verify --trusted-commit <full-sha>` for artifact-only validation | Bounded S2-07 completion of the issue's command surface; the release actor must be recorded on the release event, and trusted-snapshot validation cannot infer trust from HEAD, a branch, an abbreviated id, or local digest agreement |
| 2026-09-26 | Invoke Git with `--no-replace-objects` and a neutralized `GIT_*` environment, and require the local artifact tree to match the pinned tree byte for byte | Review-driven hardening of the YWAY-D003 trusted snapshot root; local `.git` state must never substitute for the caller-supplied commit |
