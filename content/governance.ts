...
import { readSnapshotIndex } from "./snapshot-index.js";
import {
  digestFile,
  displayPath,
  loadPackState,
...
export function attestationBindsEvent(
...
/**
 * Requires a release manifest to bind one exact immutable Pack version and to
 * agree with the source on fixture classification. This is a product guardrail,
 * so it lives in one place rather than being restated per command.
 */
export function assertReleaseManifestBindsVersion(
  ...
  if (
    manifest.packId !== scope.packId ||
    manifest.packVersion !== scope.packVersion ||
    manifest.contentDigest !== scope.contentDigest
  ) {
    throw new StrictValidationError([
      {
        path: ["releaseManifest"],
        message: `release manifest at ${label} does not bind ${scope.packId} version ${scope.packVersion} with contentDigest ${scope.contentDigest}`,
      },
    ]);
  }
  if (manifest.fixtureOnly !== scope.fixtureOnly) {
    throw new StrictValidationError([
      {
        path: ["releaseManifest", "fixtureOnly"],
        message: `release manifest at ${label} declares fixtureOnly ${manifest.fixtureOnly} but the source for ${scope.packId} version ${scope.packVersion} declares fixtureOnly ${scope.fixtureOnly}; fixture isolation must match`,
      },
    ]);
  }
...
export interface VerifiedVersionGovernance {
  readonly packId: string;
  readonly packVersion: number;
  readonly contentDigest: string;
  readonly source: PackSource;
  readonly localizedContent: LocalizedContent | undefined;
  ...
  readonly retirement: RetirementRecord | undefined;
  readonly notice: RetirementNotice | undefined;
  readonly releaseManifest: ReleaseManifest | undefined;
}

/**
 * Validates one immutable Pack version's repository records.
 *
 * `content:status`, `content:release`, and `content:verify` all depend on exactly
 * the same governance rules, so the checks live here rather than being restated
 * per command. Every failure throws a `StrictValidationError` (exit code 1).
 */
export function verifyVersionGovernance(
  repositoryRoot: string,
  packId: string,
  version: number,
): VerifiedVersionGovernance {
  ...
  if (existsSync(recordPath)) {
    retirement = strictParse(retirementRecordSchema, readJsonFile(recordPath, "retirement record"));
  ...
  if (wasReleased && retiredEvent !== undefined) {
    ...
    notice = strictParse(retirementNoticeSchema, readJsonFile(noticePath, "retirement notice"));
  ...
  if (wasReleased) {
    ...
    if (!existsSync(manifestPath)) {
      throw new StrictValidationError([
        {
          path: ["releaseManifest"],
          message: `released version ${packId} version ${version} has no release manifest at ${displayPath(repositoryRoot, manifestPath)}; a release write may have been interrupted`,
        },
      ]);
    }
  ...
  return {
    ...
    contentDigest: contentDigestHex,
    source,
    localizedContent: state.localizedContentByVersion.get(version),
    provenanceLog: log,
    status,
    attestations,
    retirement,
