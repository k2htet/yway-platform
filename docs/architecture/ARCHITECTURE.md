...
For Stage 2, YWAY-D003 adopts immutable positive-integer versions, fresh version-scoped gates after every source or localization change, repository-local lifecycle records, and deterministic artifact snapshots bound to a protected Git tree. These are bounded content-pipeline choices, not permanent production CMS, storage, identity, distribution, application, or service boundaries. (YWAY-P019, YWAY-E005, YWAY-D003)
...
Stage 2 practitioner eligibility is synthetic and repository-governed: manually verified, active/current, occupation-scoped, and supported only by non-sensitive evidence references. It validates the fixture workflow but does not establish production practitioner identity, authentication, credential verification, or broader vetting architecture. (YWAY-D003)
...
For Stage 2, Operations uses repository commands and protected review to enforce founder/practitioner sequencing, localization, content-accessibility, sponsorship, fixture-isolation, release, and retirement gates. These controls govern synthetic content artifacts only and do not establish a production Admin UI, authorization system, deployment model, or public-release process. (YWAY-D003)
...
YWAY-D003 selects YAML plus strict schemas, repository-local workflow, immutable versions, exact digest-bound review, and deterministic JSON artifact snapshots for Stage 2. Every source or localization change requires a fresh version and review path. Released-version retirement uses an immutable notice in a canonical snapshot index whose presence or absence is verified against a trusted protected Git tree. Production CMS/workflow, distribution, trusted-root acquisition, and identity/authorization mechanisms remain deferred. (YWAY-P019, YWAY-E005, YWAY-D003)
...
| Experiment structure and outcome           | YWAY-P002, YWAY-P003            | Identified experiments have six parts and increase real-world exposure before commitment; meaningful completion includes realistic task, noticing, reflection, and next fork; a started next action remains distinct from an option selected; self-report does not become verified evidence |
| Guided post-trial paths                    | YWAY-P013                       | Try-another and pause remain available; deeper and compare appear when useful, without a comparison gate                                                                                                                                                                                    |
| First value without login                  | YWAY-P011, YWAY-P012            | A short realistic trial yields a useful insight and offers a next step without authentication; public access remains 18+                                                                                                                                                                    |
